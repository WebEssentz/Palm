...
export const getAllForUser = query ({
    ...
    handler: async (ctx, { userId }) => {
        return await ctx.db
            .query("subscriptions")
            .withIndex("by_userId", (q) => q.eq("userId", userId))
            .collect() // collect all matching subscriptions into an array
    }
})
...
export const grantCreditsIfNeeded = mutation({
    ...
    handler: async (ctx, args) => {
        const { subscriptionId, idempotencyKey, amount, reason } = args
        ...
        if (grant <= 0) return { ok: true, skipped: true, reason: 'no-credits' }
        const next = Math.min(
        ...
        return { ok: true, granted: grant, reason: 'granted', balance: next }
    }
})
...
export const consumeCredits = mutation({
    ...
    handler: async (ctx, args) => {
        const { userId, amount, reason, idempotencyKey } = args

        if (amount <= 0) return { ok: false, error: 'no-credits' }
